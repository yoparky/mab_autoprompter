...
async def main():
    ...
    for _ in range(config["tree_generative_iteration"]):
        try:
            ...
            random_int = random.randrange(len(hardest_shots)) if hardest_shots else 0 # This line is problematic: empty range for randrange(). UPDATE: Fixed, need to check if sample can take 0 generated by here
